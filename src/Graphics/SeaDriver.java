...
/**
 * The SeaDriver class represents a thread for driving a sea vehicle.
 * It extends the Thread class and implements Serializable.
 */
public class SeaDriver extends Thread implements Serializable {
    private DialogThread currentThread;
    /**
     * Constructs a SeaDriver object with the specified DialogThread.
     * It increments the sea counter.
     *
     * @param thread the DialogThread associated with the driver
     */
    public SeaDriver(DialogThread thread) {
        super();
        currentThread = thread;
    }
    /**
     * Overrides the run() method of the Thread class.
     * Performs the driving operation for the sea vehicle.
     */
    @Override
    public void run() {
        ...
        if (!currentThread.current_gui.getAgency().getVehicles()[Math.min(currentThread.current_gui.getAgency().getVehicles().length - 1, currentThread.vehicle_index)].equals(currentThread.currentVehicle)) {
            ...
            Timer timer = new Timer(3000, e -> {
                currentThread.dialog.dispose(); DialogThread.openMissionsCounter -= 1;
            });
